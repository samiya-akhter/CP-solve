...
Quack will operate on each tower **exactly once**, in any order he chooses. The operation on tower `i` is as follows:
 ...
 - Quack climbs to the top of tower `i` and shoots a laser to the right, cutting the first taller tower it hits down to the same height as tower `i`.Formally, let `j` be the smallest index such that `j > i` and `a_j > a_i`, where `a_i` and `a_j` are the current heights of the towers. If such `j` exists, then `a_j` is replaced with `a_i`. Otherwise, nothing happens.
...
The following line contains `n` integers `a_1, a_2, …, a_n` (`1≤ a_i≤ 1000`) — the heights of the towers.
...
> 🔗 [View on Codeforces](https://codeforces.com/contest/2237/problem/A)
